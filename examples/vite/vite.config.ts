    ...
    resolve: {
      alias: usePublished
        ...
        : [
            ...
            {
              find: '@ucontract/docx-editor-react',
            ...
            {
              find: '@ucontract/docx-editor-agents/react',
              replacement: path.join(monorepoRoot, 'packages/agents/src/react.ts'),
            },
            {
              find: '@ucontract/docx-editor-agents/server',
            ...
            {
              find: '@ucontract/docx-editor-core/mcp',
              replacement: path.join(monorepoRoot, 'packages/core/src/mcp/index.ts'),
            },
            // Wildcard alias for deep core imports (e.g. @ucontract/docx-editor-core/utils/docxInput)
            {
              find: /^@eigenpal\/docx-editor-core\/(.+)/,
              replacement: path.join(monorepoRoot, 'packages/core/src/$1'),
            },
            // Exact match for bare @ucontract/docx-editor-core (must come AFTER the prefix match above)
            {
              find: /^@eigenpal\/docx-editor-core$/,
