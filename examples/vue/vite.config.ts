...
// resolve `@ucontract/docx-editor-vue` + `/agents/*` through node_modules
// (the workspace's published dist/) so the deployment shows the real
// consumer experience. Core deep paths (e.g.
// `@ucontract/docx-editor-core/headless`) are kept aliased to source in
...
const coreAliases = [
  {
    find: '@ucontract/docx-editor-core/headless',
    replacement: path.join(monorepoRoot, 'packages/core/src/headless.ts'),
  },
  {
    find: '@ucontract/docx-editor-core/core-plugins',
  ...
  // Exact match for bare @ucontract/docx-editor-core (must come AFTER prefix match)
  {
    find: /^@eigenpal\/docx-editor-core$/,
  ...
  resolve: {
    alias: usePublished
      ...
      : [
          ...
          {
            find: '@ucontract/docx-editor-vue/styles.css',
